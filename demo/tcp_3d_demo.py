...
class Demo3D(tk.Tk):
    ...
    # ---------- 2. 连接数据端 (LISTEN_PORT 发送指令) ----------
        elif mtype == 'TIMETABLE_STATE':
            self._log_json('recv', msg)
            flight_time = data.get('flightTime', '?')
            flight_states = data.get('flightStates', {})
            state_count = 1 if isinstance(flight_states, dict) and flight_states else len(flight_states) if isinstance(flight_states, list) else 0
            self.info_label.config(text=f'鏃堕棿琛ㄦ暟鎹? {flight_time}  鐘舵€佽褰? {state_count}')
            self.info_label.config(text=f'TIMETABLE_STATE: {flight_time}  records={state_count}')
